               ...
               <!-- content here -->
                    <div class="row">
                        ...
                        <div class="card">
                                    ...
                                    <?php
                                       if(isset($_POST['save'])){
                                        if(empty($_POST['pass']) || empty($_POST['username']) || empty($_POST['password'])){
                                            echo '<div class="alert alert-danger">All fields are required</div>';
                                        }else{
                                        if(password_verify($_POST['pass'],$_SESSION['user']['password'])){
                                           $pass = password_hash($_POST['password'],PASSWORD_DEFAULT);
                                           $token = password_hash($_POST['password'].$_POST['username'],PASSWORD_DEFAULT);
                                            $sql = "INSERT INTO `users`(`username`, `password`, `token`) 
                                            VALUES ('{$_POST['username']}','{$pass}','{$token}')";
                                          $id = $db->writedb($sql);
                                          if(!$id){
                                            echo '<div class="alert alert-danger">Tên người dùng đã được sử dụng</div>';
                                          }else{
                                            echo '<div class="alert alert-success">Đã thêm người dùng</div>';
                                          }
                                        }else{
                                          echo '<div class="alert alert-danger">Mật khẩu cũ sai</div>';
                                        }
                                        }
                                      }
                                      if(isset($_GET['del'])){
                                          $d=base64_decode($_GET['del']);
                                         $db->writedb("DELETE FROM `users` WHERE `id`='{$d}'");
                                      }
                                      $users = $db->readdb("SELECT * FROM `users`");
                                      ?>
                                    ...
                                    <form method="POST" style="width: 100%;">
                                        ...
                                        <div class="form-group col-md-3">
                                            ...
                                            <input type="text" name="username" value="<?=isset($u)?$u['username']:''?>" class="form-control">
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label class="mb-1"><strong>Mật khẩu cũ</strong></label>
                                            <input type="password" name="pass"  class="form-control">
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label class="mb-1"><strong>Mật khẩu</strong></label>
                                            <input type="password" name="password" <?=isset($u)?$u['password']:''?> class="form-control">
                                        </div>
                                       <div class="form-group col-md-2">
                                           <label for=""></label>
                                       <button class="btn btn-success btn-xs" style="margin-top: 25px;" name="save" type="submit">Lưu</button>
                                       </div>
                                        </div>
                                    </form>
                                    </div>
                                            ...
                                            <table class="table table-responsive-sm mb-0" id="datat">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th><strong>MẬT KHẨU MỞ KHÓA</strong></th>
                                                  ...
                                                  foreach($users as $user)  {
                                                    ?>
                                                     <tr>
                                                         <td><?=++$c?></td>
                                                         <td><?=$user['username']?></td>
                                                         <td><input type="password" value="<?=$user['password']?>" style="background-color: #1E2746 !important;border:none;outline:none" readonly></td>
                                                         <td><input type="password" value="<?=$user['token']?>" style="background-color: #1E2746 !important;border:none;outline:none" readonly></td>
                                                         <td>
                                                         
                                                         <a href="users?del=<?=base64_encode($user['id'])?>"  class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>
                                                         </td>
                                                     </tr>
                                                    <?php
                                                  }  
        ...
        <div class="popbody" style="max-width: 70%;">
          ...
          <div class="card">
                            <div class="card-header">
                                <h4 class="card-title" style="width: 100%;">Add New <small class="clse" style="float: right;cursor: pointer;">close</small></h4>
                            </div>
...
?>
                                    ...
                                    <form method="POST" id="dat" onsubmit="return false" enctype="multipart/form-data">
                                     <div class="form-row">
                                            ...
                                            <button type="submit" id="addc" style="float: right; margin-top: 20px;" class="btn btn-success">Further</button>  
                                            </div>
                                     </div>
                                    </form>
                                    </div>
                                ...
                                <button class="btn btn-danger clse" style="float: right; margin-top: 20px; margin-left: 10px;" >Close</button>
                                <button type="submit" id="f2" style="float: right; margin-top: 20px;" class="btn btn-success">Add</button>
